...
export interface HistoryScopeOptions { config: string; task: string; releasedSession?: string; operatorProfile?: string; ancestryCheck?: string; verificationId?: string }
...
export async function historyConnection(options: HistoryScopeOptions) {
  ...
  const actor = options.operatorProfile ? { kind: "operator" as const } : options.releasedSession
    ...
    : { kind: "liveSession" as const, verificationId: options.verificationId ?? (await resolveCaller(config)).verificationRequestId };
