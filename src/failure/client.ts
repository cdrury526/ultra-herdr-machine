...
import { failureApi, failureResultSchemas, stopApi, stopResultSchemas, canonicalJson, jsonDigest, parseJson } from "@ultra-herdr/api";
...
function failureError(error: unknown, kind: "failure" | "stop"): never {
  ...
  throw new FailureError(reportFailure(error).message.replace(/\breport\b/gi, `${kind} request`));
}
...
function display(result: ReturnType<typeof failureResultSchemas.status.parse>, kind: "failure" | "stop") {
  const { errorJson, ...status } = result;
  // Stored field errors can contain user-authored names. Do not print their raw JSON.
  let error: string | undefined;
  if (errorJson) {
    try { error = reportFailure(new ConvexError(JSON.parse(errorJson))).message.replace(/\breport\b/gi, `${kind} request`); }
    catch { error = `Preparation could not finish. Check the ${kind} brief contracts and current caller authority.`; }
  }
...
export async function requestFailure(options: FailureOptions, kind: "failure" | "stop" = "failure") {
  ...
  try {
    ...
    return display(schemas.request.parse(await client.mutation(api.request,
...
export async function failureStatus(options: Pick<FailureOptions, "config" | "requestFile">, kind: "failure" | "stop" = "failure") {
  ...
  try {
    ...
    return display(schemas.status.parse(await client.query(api.status,
      { verificationId: caller.verificationRequestId, requestId: saved.requestId })), kind);
  } catch (error) { failureError(error, kind); }
}
