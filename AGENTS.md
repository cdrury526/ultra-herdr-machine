...
command execution and public dispatch are implemented; end-to-end orchestration
acceptance remains in progress. Source/config files must remain at most
...
All coding managers, workers and reviewers run as interactive foreground TUIs in
Herdr panes, never print/headless/background agents. User-required launch flags:
`codex --yolo`, `claude --dangerously-skip-permissions`, `cursor-agent --yolo`,
